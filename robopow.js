const Robopow = (function () {
  ...
  async function findNonce(token, challenge, zerobits) {
    ...
    while (true) {
      ...
      const msgUint8 = new TextEncoder().encode(
        stringNonce.toString() + token + challenge,
      );
      const hashBuffer = await crypto.subtle.digest("SHA-512", msgUint8);
      const hashArray = Array.from(new Uint8Array(hashBuffer));
      const bits = hashArray.map((n) => numToHex(n, 2, 8)).join("");
    ...
    console.debug(
      `Robopow round took ${duration}ms to find ${zerobits} zerobits`,
    );
    return nonce;
  }
  ...
  async function verifyCaptcha(apiUrl, config) {
    ...
    const challengeRequest = await fetch(
      `${apiUrl}/v0/challenge?${configParams}`,
    );
    ...
    const duration = end - start;
    console.log(
      `Robopow round took ${duration}ms to solve ${challenges.length} challenges`,
    );
